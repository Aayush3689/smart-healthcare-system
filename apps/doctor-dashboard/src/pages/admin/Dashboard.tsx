...
export const Dashboard: React.FC = () => {
  ...
  const handleCloseModal = () => {
    if (showModal === 'patientAssessment') {
      resetAssessmentForm();
    }
    setShowModal(null);
  };
  ...
  const toggleSpeechRecording = () => {
    ...
    if (!SpeechRecognition) {
      alert("Live speech recognition is not supported in this browser version. Try using Google Chrome or Microsoft Edge.");
      return;
    }
    ...
    if (isRecording) {
      ...
      startSpeechRecording(SpeechRecognition);
    }
  };

  const startSpeechRecording = (SpeechRecognitionClass: any) => {
    try {
      ...
      recognition.onresult = (event: any) => {
        ...
        setPatientForm(prev => ({
          ...prev,
          voiceText: prev.voiceText ? `${prev.voiceText} ${transcript}` : transcript
        }));
      };
  ...
  const handleAddAsha = (e: React.FormEvent) => {
    ...
    handleCloseModal();
    showToast("ASHA Worker registered successfully!");
  };
  ...
  const handleAddHospital = (e: React.FormEvent) => {
    ...
    handleCloseModal();
    showToast("Hospital/PHC registered successfully!");
  };
  ...
  const handleAssignDoctor = (doctorId: string) => {
    ...
    handleCloseModal();
    showToast("Doctor appointment booked successfully!");
  };
  ...
  const handleRunClinicalTriage = async () => {
    ...
    try {
      ...
      // Clinical Vitals Safeguard Override
      const temp = Number(patientForm.temperature) || 98.6;
      const sysBP = Number(patientForm.bloodPressure.split('/')[0]) || 120;
      const isCriticalVitals = Number(patientForm.oxygenSaturation) < 94 || temp > 101 || sysBP >= 140 || patientForm.symptomsChecklist.includes('Shortness of breath');

      if (isCriticalVitals) {
        const extraReasons: string[] = [];
        if (Number(patientForm.oxygenSaturation) < 94) extraReasons.push(`Low SpO2 (${patientForm.oxygenSaturation}%)`);
        if (sysBP >= 140) extraReasons.push(`High BP (${patientForm.bloodPressure})`);
        if (patientForm.symptomsChecklist.includes('Shortness of breath')) extraReasons.push('Shortness of breath');

        setAiTriageResult({
          clinical_risk: 'HIGH',
          clinical_score: Math.max(res.clinical_score || 0, 75),
          clinical_reasons: Array.from(new Set([...(res.clinical_reasons || []), ...extraReasons]))
        });
      } else {
  ...
  // Save Patient Assessment & Trigger Toast
  const handleSavePatientAssessment = (e: React.FormEvent) => {
    ...
    handleCloseModal();
    showToast(`Patient "${patientForm.name}" Assessment Saved Successfully!`);
  };
  ...
  return (
    <div className="space-y-6 bg-slate-50 min-h-screen p-2 sm:p-4 text-slate-800 font-sans relative">
      
      {/* Toast Notification Banner */}
      {toastMessage && (
        <div className="fixed top-5 right-5 z-50 bg-slate-900 text-white px-5 py-3.5 rounded-2xl shadow-2xl border border-slate-700 flex items-center gap-3 animate-bounce">
          <div className="p-1.5 bg-emerald-500 rounded-full text-slate-950">
      ...
      {showModal === 'patientAssessment' && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4 z-50 overflow-y-auto">
          <form onSubmit={handleSavePatientAssessment} className="bg-white rounded-3xl p-6 sm:p-8 w-full max-w-4xl shadow-2xl border border-slate-200 max-h-[90vh] overflow-y-auto space-y-6">
            ...
            {/* Modal Title Bar with Close Action */}
            ...
            {/* SECTION 6: Prescription OCR & Live Mic Voice Input */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-slate-50 border border-slate-200 rounded-2xl">
              ...
              <div>
                ...
                <textarea readOnly placeholder="Extracted prescription text will appear here..." value={patientForm.ocrText} className="w-full p-2 text-xs border rounded-xl bg-white h-20" />
              </div>

              {/* Live Interactive Microphone Speech-to-Text */}
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  >
                    {isRecording ? (
                      <>
                        <Square className="w-3 h-3 fill-current" /> Recording... (Click to Stop)
                      </>
                    ) : (
                      <>
                        <Mic className="w-3 h-3" /> Start Mic Recording
                      </>
                    )}
                  </button>
                </div>
                <textarea 
                  placeholder={isRecording ? "Listening... Speak into your microphone..." : "Transcribed spoken text will automatically appear here..."} 
                  value={patientForm.voiceText} 
                  onChange={e => setPatientForm({...patientForm, voiceText: e.target.value})}
                  className="w-full p-2.5 text-xs border border-slate-200 rounded-xl bg-white h-20 focus:outline-emerald-500" 
                />
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-4 border-t border-slate-100">
              <button type="button" onClick={handleCloseModal} className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-2xl transition">Cancel</button>
              <button type="submit" className="flex-1 py-3 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold rounded-2xl transition shadow-md">Save Patient Record</button>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-slate-100">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={handleCloseModal} className="p-1 rounded-lg hover:bg-slate-100"><X className="w-4 h-4 text-slate-500" /></button>
            </div>
            ...
            <button onClick={handleCloseModal} className="mt-4 w-full py-2 bg-slate-100 text-slate-700 text-xs font-bold rounded-xl">Cancel</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <form onSubmit={handleAddDoctor} className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl border border-slate-100 space-y-4">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={handleCloseModal} className="flex-1 py-2 bg-slate-100 text-slate-700 text-xs font-bold rounded-xl">Cancel</button>
