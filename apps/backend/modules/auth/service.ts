  ...
  public async requestOtp(email: string): Promise<void> {
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development" || process.env.NODE_ENV === "test") {
        console.warn(`[DEV OTP Warning] Could not send SMTP email, but generated code ${otp} for testing.`);
        return;
      }
      throw new AppError(
        "Email delivery is not configured. Verify the SMTP credentials and try again.",
        503,
